...
// logger.level = 'debug';
logger.level = 'info';
let {pickRandomNFrom} = require('./utils')
...
SequenceCoder.prototype._extend = function (listOfStates, x) {
    ...
    if(listOfStates.length !== 0) {
        ...
        if (this._ignite(x).length === 0) { 
            // logger.debug("improving transition: ", JSON.stringify(listOfStates), x);
...
SequenceCoder.prototype.consume = function (pattern, learningIsOn = false) {
    return this._consume(pattern, learningIsOn); 
};

/** 
 * @function 
 */
SequenceCoder.prototype._consume = function (pattern, learningIsOn = false) {
    logger.debug("consume", JSON.stringify(pattern));
    let X              = pattern; 
 ...
 * @function 
 */
SequenceCoder.prototype._interpolatingPrediction = function (state) {
    let currentPrediction = this._computePrediction(state);
    let pattern = this._baseProjection(state);
    let selfPredSegs = pattern
                    .filter(x => x in currentPrediction)
                    .map(x => currentPrediction[x]['segments'])
                    .reduce((a,b) => a.concat(b), []);
    let reconstr = selfPredSegs
                    .map(s => this.stateSpace.feedingCells[s])
                    .reduce((a,b) => a.concat(b), []);

    let extendedState = Array.from(new Set(state.concat(reconstr)));

    return this._computePrediction(extendedState);
};

/** 
 * @function 
 */
SequenceCoder.prototype._consumeWithInterpolation = function (pattern, learningIsOn = false) {
    let X              = pattern; 
    let lastState      = this.lastState;
    let lastPrediction = this.lastPrediction;
    ...
    let newState      = this._lift(pattern, lastPrediction);
    let newPrediction = this._interpolatingPrediction(newState);
    let newOutput     = this._computeOutput(this.lastPrediction, this.lastScore, newState);
 ...
 * @function 
 */
SequenceCoder.prototype._learnBranch = function (patterns) {
