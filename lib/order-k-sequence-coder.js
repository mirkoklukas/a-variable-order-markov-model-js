...
var StateSpace = require('./state-space');
...
SequenceCoder.prototype.resetState = function () {
    this.lastPattern = [];
    this.lastState   = [];
    this.lastPrediction = {};
    this.lastScore  = {};
...
SequenceCoder.prototype._lift = function (pattern, lastPrediction) {
    let state  = [];
    logger.debug("lift: ",  JSON.stringify(pattern), JSON.stringify(lastPrediction) );
    ...
    logger.debug("lifted state:", JSON.stringify(state));
    return state;
};
...
SequenceCoder.prototype._extend = function (listOfStates, x) {
    "use strict";
    logger.debug("extend graph: ", JSON.stringify(listOfStates), x);

    let tilde_x_new; 
    if(listOfStates.length !== 0) {
        tilde_x_new = this.stateSpace.createStateOver(x);
...
SequenceCoder.prototype._improveTransition = function (pattern, lastState, lastPrediction) {
    "use strict";
    let X  = pattern; 
    logger.debug("improving transition:", JSON.stringify(lastState), JSON.stringify(pattern));
    let extension = {}
    if(lastState.length == 0) return extension;
    for (let i = X.length - 1; i >= 0; i--) {
        ...
        if(!(x in lastPrediction)) { 
            ...
            extension[x] = { cells: [this._extend(domain , x)] };
...
SequenceCoder.prototype._computePrediction = function (listOfStates) {
    "use strict";
    logger.debug("compute prediction:", JSON.stringify(listOfStates));
    let prediction = {}
    let weights = {}
    ...
    this.stateSpace.increaseActivities(activeSegments, this.incr)
 ...
 * @function 
 */
SequenceCoder.prototype.consume = function (pattern, learningIsOn = false, continuationIsOn = false) {
    return this._consume(pattern, learningIsOn, continuationIsOn); 
};

/** 
 * @function 
 */
SequenceCoder.prototype._consume = function (pattern, learningIsOn = false, continuationIsOn = false) {
    logger.debug(chalk.bold("consume"), JSON.stringify(pattern));
    let X              = pattern; 
    let lastState      = this.lastState;
    let lastPrediction = this.lastPrediction;

    if(learningIsOn) {
        let extension = this._improveTransition(X, lastState, lastPrediction);
        if(continuationIsOn) {
            Object.assign(lastPrediction, extension);
        } 
    }

    logger.debug("consume -> lift");
    let newState      = this._lift(pattern, lastPrediction);

    logger.debug("consume -> predict");
    let newPrediction = this._computePrediction(newState);
    logger.debug("new prediction:", JSON.stringify(newPrediction))
    let newOutput     = this.computeOutput(this.lastPrediction, this.lastScore, newState, newPrediction);
...
SequenceCoder.prototype.learn = function (pattern) {
    return this._consume(pattern, true);
};

/** 
 * @function 
 */
SequenceCoder.prototype.learnBranch = function (patternBranch) {
    patternBranch.forEach(pattern => {
        this._consume(pattern, true, true);
    })
    return this;
};
 ...
 * @function 
 */
SequenceCoder.prototype.computeOutput = function (lastPrediction, lastScore, state) {
    "use strict";
};
 ...
 * @function 
 */
SequenceCoder.prototype._basePrediction = function (lastPrediction) {

    var basePrediction = {};
    let stateSpace = this.stateSpace;
    logger.debug(chalk.blue("segment predictor: based on prediction"), JSON.stringify(lastPrediction));
    Object.keys(lastPrediction).forEach( x => { 
        let predictionsOfActiveSegment = lastPrediction[x]["segments"].map(seg => segmentPredictor(stateSpace, seg)) ;
        logger.debug(chalk.blue("segment predicton:"), "for input bit:", x, ", votes:",JSON.stringify(predictionsOfActiveSegment))

        // logger.debug(chalk.blue("...."), predictionsOfActiveSegment.map(seg => stateSpace.segmentWeigts[seg]));
        if( majorityVoteBinary(predictionsOfActiveSegment) == 1) {
            basePrediction[x] = 1;
        }
    });

    logger.debug(chalk.blue("Final base predicton { inputbit: predicted_activity }"), JSON.stringify(basePrediction));
...
SequenceCoder.prototype._baseProjection = function (listOfStates) {
    ...
    return Array.from(new Set(listOfStates.map(s =>  this.stateSpace.getBase(s))));
};
